...
import com.biz.memo.adaptor.MemoViewAdaptor;
import com.biz.memo.adaptor.MemoViewModel;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    MemoViewAdaptor view_adapter=null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RecyclerView memo_list_view=findViewById(R.id.memo_list_view);

        //recyclerView에 데이터를 표시하기 위해서
        // Adapter를 부착하는 부분
        view_adapter=new MemoViewAdaptor(this);
        memo_list_view.setAdapter(view_adapter);

        /*
        DB 연동을 위한 준비
        2.2.0 : memoViewModel=new ViewModelProvider(this).get()
        2.2.2:
         */
        // LifeCycle 2.2.0의 Viewmodelprovider 사용
        ...
        observ()메서드가 알람을주고 onChanged 이벤트가 발생을 한다.
       ...
       MemoViewAdaptor.OnDeleteButtonClickListner deleteBtnEvent=new MemoViewAdaptor.OnDeleteButtonClickListner() {
           @Override
    ...
    public void onClick(View v) {
        ...
        MemoVO memoVO=MemoVO.builder().m_date(sd.format(date))
                .m_time(st.format(date))
                .m_text(m_memo_text).build();

        // memoViewModel 의 insert 메서드를 호출하여 DB에 memoVO 데이터를 저장
