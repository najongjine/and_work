package com.biz.myapp.com.biz.myapp.utils;
...
그때 자기 자신의 클래스를 사용하지 않고 View 클래스의 요소들을 상속 받아서
클래스를 작성하는 것이 원칙이다.
 */
public class MyViewClass implements View.OnClickListener {
    ...
    public void onClick(View v) {
        /*
        이벤트가 발생하면 onClick() method가 실행(호출) 될것이고
        누가 호출했는가를 알고싶을때 v.getId() method를 사용하면 호출한 view(comp)의 id값을 얻어
        올수 있다.
         */
        String msg="반갑2";
        if(v.getId()== R.id.btn1){
            ...
            /*
            이벤트가 살생한 view(compo)로부터 어떤값을 얻어 오고자 하면 해당 view로 형변환(type cast)
            를 한번 수행해서 객체를 만들고
